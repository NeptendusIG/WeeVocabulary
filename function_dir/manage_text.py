...
def establish_files():
    ...
    FICHIERS """
    File.create_file(SETTINGS_PATH, settings_dict)  # create settings jsonfile
    source_path = File.JsonFile.get_value_jsondict(SETTINGS_PATH, "global")
    File.create_file(source_path, can_make_dirs=False)
    data_path = File.JsonFile.get_value_jsondict(SETTINGS_PATH, "data")
    File.create_file(data_path, default_content=DEFAULT_CSV)
    File.create_file("data/backup/en_backup.txt")
    File.create_file("data/backup/fr_backup.txt")
...
"""def saved_word_to_backup(word, field=None, backup_path=None):
    ...
    if backup_path is None and field is not None:
        settings_of_chosen_field = File.JsonFile.get_value_jsondict(SETTINGS_PATH, field, handle_keyERROR=True)
