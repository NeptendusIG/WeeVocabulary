...
from weevocabulary.function_dir.manage_dataframe import import_data_csv, get_ready_words, add_new_words
from weevocabulary.function_dir.launch_interfaces import add_three_board
from weevocabulary.function_dir.anki_maker import make_ankipackage_from_bituples
# typing
...
def choose_location_for_input_words():
    ...
    File.JsonFile.set_value_jsondict(SETTINGS_PATH, "global", new_file_path,
                                     can_modify_key=True, can_add_key=False)
    logger.info(f"OP:Change Location: CHANGED ({new_file_path})")
    File.create_file(new_file_path, can_make_dirs=False)
...
def get_data():
    ...
    # 1 - New words
    source_path = File.JsonFile.get_value_jsondict(SETTINGS_PATH, "global", handle_keyERROR=False)
